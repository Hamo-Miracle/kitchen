...
class Crb_Initialize_Class extends Crb_Initialize_Base {
	...
	========================================================================== */
			...
			========================================================================== */
				...
				Container::make( 'post_meta', sprintf( __( '%s Dates settings (Admin only)', 'crb' ), $this->singular_name ) )
					...
					->add_fields( array(
                        ...
                        //add a field for enrolled students
                        Field::make( 'text', 'crb_enrolled_student', __( 'Enrolled Student', 'crb' ) ),
		...
		function register_admin_columns() {
			...
			//enrolled student
            if ( Crb_Current_User()->is( 'crb_facilitator' ) || Crb_Current_User()->is('administrator')){
                $new_columns[] = Carbon_Admin_Column::create( 'Enrolled Students' )
                    ->set_name( 'crb-enrolled-student-column' )
                    ->set_field( '_crb_enrolled_student' )
                    ->set_callback(array( $this, 'column_callback_enrolled_student'));
            }
	...
	========================================================================== */
        ...
        /**
         * Display the enrolled numbers associated with the current Class
         */
        function column_callback_enrolled_student( $post_id ) {
            $enrolled_student = get_post_meta( $post_id, '_crb_enrolled_student', true);

            if( empty($enrolled_student) )
                return 0;
            else
		 ...
		 * Display all Dates assigned to the current Class, that are scheduled
		 */
		function column_callback_get_dates_for_download( $post_id , $facilitator_id = null ) {
			$class = new Crb_Class( $post_id );
			$dates = $class->get_dates();
			...
			$is_main_facilitator = $class_main_facilitator_id && $class_main_facilitator_id == Crb_Current_User()->get_id();
			// Change the array from "IDs" to "Titles"
			array_walk( $dates, function( &$date_id ) use( $is_main_facilitator , $facilitator_id) {
				$date = new Crb_Date( $date_id );
				$is_admin = Crb_Current_User()->is( 'administrator' );
				$is_facilitator = Crb_Current_User()->is( 'crb_facilitator' );
				// Skip dates for Facilitator, who is not assigned to them
				if ( $is_facilitator ) {
					$current_facilitator_id = Crb_Current_User()->get_id();
                ...
                if ($facilitator_id != $date->get_facilitator_id() and $facilitator_id != null){
                    $date_id = '';
                    return;
                }
    ...
    /**
     * Display all Dates assigned to the current Class, that are scheduled for the opportunities
     */
    function column_callback_get_dates_opportunities( $post_id ) {
        $class = new Crb_Class( $post_id );
        $dates = $class->get_dates();
...
/**
		 ...
		 */
		function column_callback_get_dates_upcoming( $post_id , $facilitator_id = null) {
			$class = new Crb_Class( $post_id );
			$dates = $class->get_dates();
