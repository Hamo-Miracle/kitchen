...
require_once AIR_WP_SYNC_PRO_PLUGIN_DIR . 'modules/user/class-air-wp-sync-user-helpers.php';
require_once AIR_WP_SYNC_PRO_PLUGIN_DIR . 'modules/user/class-air-wp-sync-user-importer.php';
require_once AIR_WP_SYNC_PRO_PLUGIN_DIR . 'modules/user/destinations/class-air-wp-sync-user-destination.php';
require_once AIR_WP_SYNC_PRO_PLUGIN_DIR . 'modules/user/destinations/class-air-wp-sync-user-meta-destination.php';

/**
 * Class Air_WP_Sync_User_Module.
 *
...
class Air_WP_Sync_User_Module extends Air_WP_Sync_Abstract_Module {
	...
	public function register_styles_scripts() {
		...
		if ( is_object( $screen ) && 'airwpsync-connection' === $screen->id ) {
			wp_enqueue_script( 'air-wp-sync-user-hooks', plugins_url( 'modules/user/assets/js/hooks.js', AIR_WP_SYNC_PRO_PLUGIN_FILE ), array( 'air-wp-sync-admin' ), AIR_WP_SYNC_PRO_VERSION, false );
		}
	}
	...
	public function get_importer_instance( $post ) {
		return new Air_WP_Sync_User_Importer( $post, $this, Air_WP_Sync_Services::get_instance()->get( 'airtable_api_client_class_factory' ), Air_WP_Sync_Services::get_instance()->get( 'filters' ) );
	}
	...
	public function register_destinations() {
		...
		new Air_WP_Sync_User_Meta_Destination( new Air_WP_Sync_Markdown_Formatter( new Air_WP_Sync_Parsedown() ), Air_WP_Sync_Services::get_instance()->get( 'attachments_formatter' ) );
	}
