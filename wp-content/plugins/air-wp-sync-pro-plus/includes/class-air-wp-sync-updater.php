...
class Air_WP_Sync_Updater {
	...
	/**
	 * API data (version, license key, product id...).
	 *
	 * @var array|null
	 */
	...
	/**
	 * Override WP transient information if already populated.
	 *
	 * @var bool
	 */
	 ...
	 * @param Air_WP_Sync_Options|null   $options Options.
	 * @param Air_WP_Sync_Licensing|null $licensing EDD licensing.
	 *
	 * @uses hook()
	 *
	 * @uses plugin_basename()
	 */
	public function __construct( $_api_url, $_plugin_file, $_api_data = null, $options = null, $licensing = null ) {
		...
		$this->options                  = isset( $options ) ? $options : new Air_WP_Sync_Options();
		$this->licensing                = isset( $licensing ) ? $licensing : new Air_WP_Sync_Licensing( AIR_WP_SYNC_PRO_WPC_URL, AIR_WP_SYNC_PRO_WPC_PRODUCT_ID );
	...
	public function init() {
		...
		add_action( 'in_plugin_update_message-' . AIR_WP_SYNC_PRO_BASENAME, array( $this, 'plugin_update_message' ), 10, 0 );
	}
	 ...
	 * @param array  $plugin An array of plugin data. See `get_plugin_data()`
	...
	public function show_update_notification( $file, $plugin ) {
		...
		/* translators: the plugin name. */
			esc_html__( 'There is a new version of %1$s available.', 'air-wp-sync' ),
			esc_html( $plugin['Name'] )
		);

		if ( ! current_user_can( 'update_plugins' ) ) {
			echo ' ';
			esc_html_e( 'Contact your network administrator to install the update.', 'air-wp-sync' );
		} elseif ( empty( $update_cache->response[ $this->name ]->package ) && ! empty( $changelog_link ) ) {
			echo ' ';
			printf(
			/* translators: 1. opening anchor tag, do not translate 2. the new plugin version 3. closing anchor tag, do not translate. */
				esc_html__( '%1$sView version %2$s details%3$s.', 'air-wp-sync' ),
			...
			echo ' ';
			printf(
			/* translators: %1$s = opening anchor tag, %2$s = version, %3$s = closing anchor tag, %4$s = opening anchor tag, %5$s = closing anchor tag */
				esc_html__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'air-wp-sync' ),
				'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
				esc_html( $update_cache->response[ $this->name ]->new_version ),
				'</a>',
			...
			printf(
				' %1$s%2$s%3$s',
				'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
				esc_html__( 'Update now.', 'air-wp-sync' ),
				'</a>'
			);
		}
	...
	/**
	 * Adds invalid license message if needed
	 */
	public function plugin_update_message() {
		$license_key    = $this->options->get( 'license_key' );
		$remote_license = $this->licensing->check_license( $license_key );
		if ( is_wp_error( $remote_license ) ) {
			echo '<br />' . wp_kses_post( $remote_license->get_error_message() );
		}
	}
	...
	public function plugins_api_filter( $result, $action = '', $args = null ) {
		...
		if ( isset( $result->sections ) && ! is_array( $result->sections ) ) {
			$result->sections = $this->convert_object_to_array( $result->sections );
		}

		// Convert banners into an associative array, since we're getting an object, but Core expects an array.
		if ( isset( $result->banners ) && ! is_array( $result->banners ) ) {
			$result->banners = $this->convert_object_to_array( $result->banners );
		}

		// Convert icons into an associative array, since we're getting an object, but Core expects an array.
		if ( isset( $result->icons ) && ! is_array( $result->icons ) ) {
			$result->icons = $this->convert_object_to_array( $result->icons );
		}

		// Convert contributors into an associative array, since we're getting an object, but Core expects an array.
		if ( isset( $result->contributors ) && ! is_array( $result->contributors ) ) {
			$result->contributors = $this->convert_object_to_array( $result->contributors );
		}
	...
	public function show_changelog() {
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'air-wp-sync' ), esc_html__( 'Error', 'air-wp-sync' ), array( 'response' => 403 ) );
		}
	...
	private function get_version_from_remote() {
		...
		$request = wp_remote_post(
			$this->api_url,
			apply_filters(
				'airwpsync/licensing/request_args',
				array(
					'timeout'   => 15,
					'sslverify' => true,
					'body'      => $api_params,
