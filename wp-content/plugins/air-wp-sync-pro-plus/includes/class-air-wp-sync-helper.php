...
class Air_WP_Sync_Helper {
	 ...
	 *
	 * @param int|string $datetime A timestamp or a datetime (@see https://www.php.net/manual/en/function.strtotime.php).
	 *
	 * @return false|string
	 */
	 ...
	 *
	 * @param int $id Importer connection id.
	 *
	 ...
	 * Get modules
	 *
	 * @return Air_WP_Sync_Abstract_Module[] $modules Registered modules.
	 */
	...
	/**
	 * Convert emoji in $data if required ($db_column in $db_table uses utf8)
	 *
	 ...
	 * @return array|mixed
	 */
	public static function maybe_convert_emoji( $data, $db_table = 'options', $db_column = 'option_value' ) {
		global $wpdb;
		...
		if ( apply_filters( 'airwpsync/should_convert_emoji', 'utf8mb4' !== $charset ) ) {
	...
	public static function generate_hash( $record, $config ) {
		$record_array = json_decode( wp_json_encode( $record ), true );
		$filtered     = self::filter_attachments_urls( $record_array );
		// Remove cs & ts query strings from urls in AT record.
		$record_json = preg_replace( '/cs=[^"]+/i', '', wp_json_encode( $filtered ) );
	 ...
	 *
	 * @param array $record_as_array Airtable record.
	 *
	 * @return array
	 */
	public static function filter_attachments_urls( $record_as_array ) {
		foreach ( $record_as_array as $key => &$value ) {
			if ( is_array( $value ) ) {
				$value = self::filter_attachments_urls( $value );
			}
		}
	 ...
	 * @param   string $current_date  Current date (mainly for tests purpose).
	 ...
	 * @return  array  $flattened  Flattened array.
	 */
	public static function array_flatten( $initial ) {
		if ( ! is_array( $initial ) ) {
