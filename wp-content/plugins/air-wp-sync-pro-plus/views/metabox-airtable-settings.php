<?php
/**
 * Display Airtable settings options: Token, base / table selection and filters.
 *
 * @package Air_WP_Sync_Pro
 */

/**
 * Metabox Airtable settings view.
 */
return function () {
	?>
	...
	<tr valign="top">
		...
		<td>
			<div :class="'airwpsync-field ' + getValidationCssClass('apiKey')">
				...
				<p class="description"><?php echo wp_kses_post( __( 'Make sure your token has the <code>data.records:read</code> and <code>schema.bases:read</code> scopes and has access to the base you want to import.', 'air-wp-sync' ) ); ?></p>
			</div>
		</td>
	</tr>
	<tr valign="top">
		...
		<td>
			<div class="airwpsync-field">
				<select
					name="app_id"
					x-cloak x-model="config.app_id"
					class="regular-text ltr"
					@change="loadAirtableTables"
					:disabled="bases.length === 0 || loadingBases"
					:class="{'airwpsync-field--invalid': hasErrors('app_id')}"
					data-rules='["selectedBaseIsMissing"]'
				>
					<option value=""></option>
				...
				<template x-for="message in getErrorMessages('app_id', false)">
					<p class="airwpsync-validation-message" x-text="message"></p>
				</template>
			</div>
		</td>
	...
	<tr valign="top">
		<th scope="row">
			<label for="enable_link_to_another_record">
				...
				<span class="airwpsync-tooltip" aria-label="
				<?php
				echo esc_attr(
					wp_kses(
						__( 'The <a href="https://www.airtable.com/guides/build/connect-data-with-linked-records" target="_blank">"Link to another record"</a> feature in Airtable enables creating relationships between records in different tables.', 'air-wp-sync' ),
						array(
							'a' => array(
								'href'   => array(),
								'target' => array(),
		...
		<td colspan="2">
			<div class="notice notice-warning inline airwpsync-connection-warning">
				<p>
					<?php
					echo wp_kses(
						__( '<strong>Warning:</strong> Including "Link to another record" field(s) may extend synchronization times.<br /> Be aware that this could <strong>significantly increase the processing time.</strong><br /> Therefore, it might generate timeouts and you may need to contact your hosting provider.', 'air-wp-sync' ),
						array(
							'strong' => array(),
							'br'     => array(),
						)
