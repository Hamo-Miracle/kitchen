             ...
             public function check_for_plugin_update($checked_data)
                 {
                     ...
                     $busy  =   get_site_transient ( 'atto-check_for_plugin_update__server_busy' );
                     if ( ! empty ( $busy ) )
                        return $checked_data;
                     ...
                     if ( isset ( $_GET['force-check'] ) && $_GET['force-check']    ===  '1' )
                        {
                            global $ATTO_UpdateData;
                            $data   =   FALSE;
                            
                            if ( is_array ( $ATTO_UpdateData ) &&    isset ( $ATTO_UpdateData['atto-check_for_plugin_update_' . md5( $request_uri )] ))
                                $data   =   $ATTO_UpdateData['atto-check_for_plugin_update_' . md5( $request_uri )];
                        }
                             ...
                             if(is_wp_error( $data ) || $data['response']['code'] != 200)
                                {
                                    ...
                                    set_site_transient( 'atto-check_for_plugin_update__server_busy' , '1', ( 60 * 60 * 4 ) +  rand ( 100, 36000 ) );
                                    
                                    return $checked_data;
                                }
                                
                             set_site_transient( 'atto-check_for_plugin_update_' . md5( $request_uri ), $data, ( 60 * 60 * 48 ) +  rand ( 100, 36000 ) );
                             
                             if ( isset ( $_GET['force-check'] ) && $_GET['force-check']    ===  '1' )
                                $WPH_UpdateData['atto-check_for_plugin_update_' . md5( $request_uri )]    =   $data;
                         }
                     ...
                     if ( $response ) 
                        {
                            ...
                            if ( isset ( $response->licence_status ) )
                                {
         ...
         function ATTO_run_updater()
             {
                ...
                add_action('in_plugin_update_message-advanced-taxonomy-terms-order/taxonomy-order.php',  array($wp_plugin_auto_update, 'in_plugin_update_message'), 10, 2);
