...
class CapabilityFilters
{
    ...
    {
        @load_plugin_textdomain('publishpress-status-capabilities', false, dirname(__FILE__) . '/languages');
    ...
    function fltMapStatusCaps($caps, $meta_cap, $user_id, $wp_args, $args = [])
    {
        ...
        if (empty($args['force']) && (!$this->do_status_cap_map || (($user_id == $current_user->ID) && $is_administrator))) {
            return $caps;
        }
